...
def plot_minEseg_prop_vs_Z(
    ...
    top_xtick_pad=10,              # vertical padding for top‐axis ticks
    top_xtick_labels=None          # list of fallback labels for missing Z's
):
    """
    Plot min E_seg vs Z for each GB, shade vertical bands for highlight_Z_list,
    draw region boundaries, and add a top axis of element symbols.
    If a Z from z_ticks is missing in df, consumes the next label from top_xtick_labels.
    """
    ...
    for gb, group in df.dropna(subset=[y_prop]).groupby("GB"):
        ...
        # find the minimum E_seg per element
        min_eseg = (
            group
            .groupby("element")
            .apply(lambda x: x.nsmallest(1, y_prop).iloc[0])
            .reset_index(drop=True)
            .sort_values(by=x_prop)
        )
    ...
    if highlight_Z_list:
        ...
        for start, end in groups:
            ax1.axvspan(start - 0.5, end + 0.5, **highlight_region_kwargs)
            ax1.axvline(start - 0.5, **highlight_region_line_kwargs)
            ax1.axvline(end + 0.5, **highlight_region_line_kwargs)

    # determine x‐ticks
    ticks = z_ticks if z_ticks is not None else sorted(df[x_prop].unique())
    ax1.set_xticks(ticks)
    ax1.tick_params(axis='x')

    # build Z → element symbol map
    sym_map = (
        df.drop_duplicates(subset=[x_prop, "element"])
          .set_index(x_prop)["element"]
          .to_dict()
    )

    # fallback iterator for any Z not in sym_map
    fallback_iter = iter(top_xtick_labels) if top_xtick_labels is not None else iter([])

    # assemble top‐axis labels
    top_labels = []
    for z in ticks:
        if z in sym_map:
            top_labels.append(sym_map[z])
        else:
            # consume next fallback label, or blank if exhausted
            top_labels.append(next(fallback_iter, ""))

    # twin top axis for element symbols
    ax2 = ax1.twiny()
    ax2.set_xlim(ax1.get_xlim())
    ax2.set_xticks(ticks)
    ax2.set_xticklabels(top_labels, fontsize=ytick_fontsize)
    # ax2.set_xlabel("Element", fontsize=xlabel_fontsize)
    ax2.tick_params(axis='x', pad=top_xtick_pad)

    # rotate first legend entry to end
...
def plot_coverage_vs_temperature(df,
    ...
    for GB, GB_df in df_ele.groupby(by="GB"):
        ...
        for temp in temperature_range:
            site_concs = calc_C_GB(temp, alloy_conc * 0.01, np.array(GB_df.full_seg_spectra.values[0]))
            temp_concs.append(site_concs.sum() / get_GB_area(df, GB=GB))
        plot_data.append((temperature_range, temp_concs, GB))
        label = f"{gb_latex_dict[GB]}"
        legend_elements.append(
            plt.Line2D([0], [0], marker='o', color='w', label=label,
                       markerfacecolor=custom_colors[GB], markersize=15, linestyle='None')
        )
    legend_elements.append(legend_elements.pop(1))

    # Plot data
    for temps, concs, GB in plot_data:
        ax1.plot(temps, concs, linewidth=8, color=custom_colors[GB])
    ...
    ax1.set_xticks(kelvin_ticks)
    ax1.set_xticklabels([f"{c}" for c in celsius_ticks], fontsize=24)
    ax1.set_xlabel("Temperature (°C)", fontsize=24)
    ...
    # Element and concentration text
    ...
    ax1.legend(handles=legend_elements,
               ...
               handletextpad=0.01, borderpad=0.05, labelspacing=0.3)

    # Save and close
    # fig.savefig(f'{save_path}/WhiteCoghlan/WhiteCoghlan_{element}_{alloy_conc}.png',
